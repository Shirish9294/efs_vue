    ...
    <v-container fluid>
      ...
      <v-row align="center" justify="center">
        <v-col cols="12" md="10" v-resize="onResize">
            ...
            <template v-slot:item="props">
              <tr>
                ...
                <td nowrap="true" align="left">
                  {{ props.item.acquired_value }}
                </td>
                <td nowrap="true" align="left">
                  {{ props.item.acquired_date }}
                </td>
                <td nowrap="true" align="left">
                  {{ props.item.recent_value }}
                </td>
                <td nowrap="true" align="left">{{ props.item.recent_date }}</td>
                <td align="center">
                  <v-icon @click="updateInvestment(props.item)"
                    >mdi-pencil</v-icon
                  >
                </td>
                <td align="center">
                  <v-icon @click="deleteInvestment(props.item)"
                    >mdi-delete</v-icon
                  >
                </td>
              </tr>
          ...
          <v-data-iterator :items="investments" hide-default-footer v-else>
                  ...
                  <v-card>
                    <v-card-title class="pb-0 pt-0 pl-0">
                      <v-col cols="9" class="text-left body-2 text-truncate"
                        >{{ item.cust_number }} - {{ item.category }}<br />
                        {{ item.cust_name }}</v-col
                      >
                      <v-col cols="3" class="text-center">
                        <v-card-actions>
                          <v-icon @click="updateInvestment(item)" class="small"
                            >mdi-pencil</v-icon
                          >
                          <v-icon @click="deleteInvestment(item)" class="small"
                            >mdi-delete</v-icon
                          >
                          <v-icon
                    ...
                    <v-list v-show="isExpanded(item)" dense>
                      ...
                      <v-list-item>
                        <v-list-item-content
                          >Acquired Value:</v-list-item-content
                        >
                        <v-list-item-content class="align-end"
                          >{{ item.acquired_value }} on
                          {{ item.acquired_date }}</v-list-item-content
                        >
                      </v-list-item>
                      <v-list-item>
                        <v-list-item-content>Recent value:</v-list-item-content>
                        <v-list-item-content class="align-end"
                          >{{ item.recent_value }} on
                          {{ item.recent_date }}</v-list-item-content
                        >
                      </v-list-item>
                    </v-list>
                  </v-card>
          ...
          <v-btn class="blue mt-4 white--text" @click="addNewInvestment"
